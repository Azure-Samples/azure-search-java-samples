...
Learn how to create, load, and query a search index with vector search on Azure AI Search using Java and the [com.azure:azure-search-documents](https://search.maven.org/artifact/com.azure/azure-search-documents) package.

This readme explains how to configure and run the sample. For details about the code, see [Quickstart: Vector search in Java](https://learn.microsoft.com/azure/search/search-get-started-vector?tabs=keyless&pivots=java).

## Prerequisites

- Azure AI Search with a Basic tier or higher (required for vector search). You can [create a search service in the portal](https://docs.microsoft.com/azure/search/search-create-service-portal).

- An available region that supports vector search. See [region availability](https://learn.microsoft.com/azure/search/vector-search-overview#availability-and-pricing) for current list.

- Optionally, Azure OpenAI Service for generating embeddings, or use the pre-computed embeddings in this sample.
...
- Creates an index with vector field configurations using HNSW algorithm.
- Loads the index with documents containing text content and pre-computed vector embeddings.
- Executes pure vector searches, single vector searches with filters, and hybrid queries combining vector and text search.
